...
import dev.patika.ecommerce.core.ulties.ResultHelper;
...
public class CategoryController {
    ...
    //Kayıt işlemini yapmak için.
    @PostMapping()
    @ResponseStatus(HttpStatus.CREATED)
    //Valid ile null olmamasını kontrol edebiliyoruz.
    public ResultData<CategoryResponse> save(@Valid @RequestBody CategorySaveRequest categorySaveRequest) {
        //Request'i category'e çeviriyoruz.
        Category saveCategory = this.modelMapper.forRequest().map(categorySaveRequest, Category.class);
        this.categoryService.save(saveCategory);

        //Category'i de response'a çeviriyoruz.
    ...
    public ResultData<CategoryResponse> get(@PathVariable("id") int id) {
        ...
        //CategoryResponse çevirmek gerekiyor.
    ...
    public ResultData<CursorResponse<CategoryResponse>> cursor(
        //eğer page yazıp sayfa sayısını yazmazsak, 0.sayfayı versin.
